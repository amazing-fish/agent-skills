...
> 修改说明：本文件从 `GitHubxsy/agent-skills` 引入，并由 `amazing-fish/agent-skills` 修改了报告输出位置和大规模 diff 处理策略。来源与上游版本见仓库根目录 `sources.lock.json`。
...
- 对 `standard` 和 `large`，不要在 HTML 中复制完整 raw diff。提供固定到 `base_sha...head_sha` 的 compare 链接、固定到 `head_sha` 的文件/行永久链接，并只内嵌支持结论所需的关键 hunks。
- 对 `small`，可以内嵌 diff，也可以使用上述固定链接；如果只给链接，要在报告中明确说明。
...
只展示短小且关键的代码片段。原始 diff 细节按报告规模策略折叠、拆入有界附录、明确省略，或用固定版本链接代替。
...
5. 核对报告模式、规模统计、缺失 patch 和省略声明；GitHub 的 `standard`/`large` 报告不得包含完整 raw diff。
