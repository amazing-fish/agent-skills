...
def _validate_trusted_url(
    ...
    *,
    ...
    try:
        ...
        raise FetchError("NETWORK", f"Invalid {label} URL: {exc}") from exc
    ...
    if (
        ...
        raise FetchError(
            "NETWORK",
            f"{label.capitalize()} URL must use HTTPS on port 443 "
            "without credentials.",
        )
...
def fetch_html(session: requests.Session, url: str) -> bytes:
    response = _request_article_with_retries(session, url)
    try:
    ...
    def _render_node(self, node: Any) -> str:
        ...
        if tag == "a":
            ...
            if not href:
                return content
            target = (
                urljoin(self.article_url, href)
                if self.article_url and not href.startswith("#")
                else href
            )
